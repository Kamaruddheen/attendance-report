...
def CreateClass(request):
    ...
    all_classrooms = ClassroomModel.objects.all().order_by('year', 'sec')
...
def ManageStudents(request, id):
    ...
    if request.method == "POST":
        ...
        if stds_obj.user_type == 3:
            stds_obj.delete()
            messages.success(
                request, "Student {} deleted Successfully".format(username))
        return redirect('classroom:manage_students', id=id, permanent=True)
    stds_obj = class_obj.students.order_by('username').values(
...
def AddStudents(request, id):
    ...
    StudentFormSet = modelformset_factory(
        User, formset=BaseStudentFormSet, form=AddStudentForm)
    ...
    if request.method == "POST" and request.POST.get('add_stud_csv_form', False):
            ...
            if not csv_file.name.endswith('.csv'):
                ...
                for row in csv_reader:
                    if row.get('username', False):
                        ...
                        initial.append({'a': 'a'})
                        messages.info(
                            request, "The column header should have username")
                        break
                # print(initial)
                # print(len(initial))
                StudentFormSet = modelformset_factory(User, fields=(
                    'username', 'email', 'first_name', 'last_name'), formset=BaseStudentFormSet, form=AddStudentForm, extra=len(initial))
        ...
        if formset.is_valid():
            ...
            user_obj = User.objects.filter(
                email__in=(ins.email for ins in instances))
