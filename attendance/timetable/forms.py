...
class TimetableForm(forms.ModelForm):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.fields['s1']=forms.ModelChoiceField(queryset=SubjectModel.objects.filter(classroom=self.choice),widget=forms.Select(attrs={'id':'s1','class':'form-control',}),empty_label='Free',required=False)
        self.fields['s2']=forms.ModelChoiceField(queryset=SubjectModel.objects.filter(classroom=self.choice),widget=forms.Select(attrs={'id':'s2','class':'form-control'}),empty_label='Free',required=False)
        self.fields['s3']=forms.ModelChoiceField(queryset=SubjectModel.objects.filter(classroom=self.choice),widget=forms.Select(attrs={'id':'s3','class':'form-control'}),empty_label='Free',required=False)
        self.fields['s4']=forms.ModelChoiceField(queryset=SubjectModel.objects.filter(classroom=self.choice),widget=forms.Select(attrs={'id':'s4','class':'form-control'}),empty_label='Free',required=False)
        self.fields['s5']=forms.ModelChoiceField(queryset=SubjectModel.objects.filter(classroom=self.choice),widget=forms.Select(attrs={'id':'s5','class':'form-control'}),empty_label='Free',required=False)
    class Meta:
